...
{
	m_endpoint = endpoint;
	Console::Log(LogAccountServer, Log, TEXT("Connected " + action::ToUnicodeString(endpoint.toString())));
}

void AccountSession::OnDisconnected(net::Endpoint endpoint)
{
	Console::Log(LogAccountServer, Log, TEXT("Disconnected " + action::ToUnicodeString(endpoint.toString())));
}
